...
\begin{document}
\maketitle
\tikzset{card/.style={rectangle,draw=black,inner xsep=0.7cm,inner ysep=0.7cm, thick,rounded corners,fill=gray!10,node distance=1.9cm}}
...
\begin{enumerate}
	\item  Sortieren Sie die folgende Liste sowohl mit Mergesort als auch mit Bubblesort.
	      ...
	      \emph{Hinweis:} Schneiden Sie die Zahlen aus, um so einfacher Umordnen zu können. Sie finden daher hier noch \copies~Kopien der Liste.
	      \begin{figure}[H]
		      \centering
		      \begin{tikzpicture}
			      \node[card] (A)  {\(15\)};
			      ...
			      \node[card,right of=F] (G) {\(17\)};
			      \node[card,right of=G] (H) {\(12\)};
			      \foreach \x in {1,...,\copies} {
					      \node[card,below of=A] (A)  {\(15\)};
					      \node[card,right of=A] (B) {\(16\)};
					      \node[card,right of=B] (C) {\(19\)};
					      \node[card,right of=C] (D) {\(11\)};
					      \node[card,right of=D] (E) {\(13\)};
		      ...
		      \end{tikzpicture}
		      \caption{Kopien der Liste zum ausschneiden}
	      \end{figure}
	      \clearpage
